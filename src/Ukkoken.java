...
public class Ukkoken
{
    ...
    public class Edge{
        ...
        public Edge(int a, int b, Node end) {
            this.a = a; this.b = b; this.end = end;
        }
        ...
        public int length() {
            return Math.min(index,b) - a + 1;
        }
        @Override
        public String toString() { return str.substring(a, Math.min(b, index)+1); }
    }
    /**
     * Node of the Ukkoken.
     */
    ...
    private int fix(int x) { return x == INF ? index : x; }
    private StringBuilder prettyFormat(StringBuilder sb, Node n, StringBuilder tab)
    {
        ...
        {
            char c = e.getFirst();
            sb.append(tab).append(c).append(" : ").append(e).append(" = ").append(e.a).
                    append(',').append(fix(e.b)).append(',').append(e.length()).append('\n');
            tab.append("  ");
    ...
    {
        for (Edge edge : n) setPrefix(edge.end,edge,len+Math.min(edge.b, index)-edge.a+1);
        if (e == null) { n.a = 0; n.b = -1; }
        else { n.b = Math.min(e.b, index); n.a = n.b - len+1; }
    }
    ...
    public static void main(String[] args)
    {
        ...
        String s = input.next();
        System.out.println(new Ukkoken(s));
